...
# to run emst, take the training file and convert it into a CSV containing only feature values for each instance in order
...
# 		my (@teste) = split " ", $features[$i];
# 		for (my $j=0; $j<=$#teste; $j++){
#                     print ($teste[$j]);
...
#
		...
		my $temp=0;
		my $deletedir = '/tmp/input';
		my $result = system("rm -rf $deletedir");
		print "Result: $result\n";
		my $result = system("mkdir $deletedir");
                
open (F11, ">/tmp/input/-1") || die ("Could not open file!"); 	# file containing cluster statistics
		...
		for (my $i = 0; $i<=$#clusters; $i++) {
			...
			if (defined $clusters[$i]) {
				...
				while (defined $current) {
					...
					print F10 "$labels[$current->[0]] $qids[$current->[0]] ";
					}
		...
		open (F5, ">$resultfolder/selectedset.tmp") || die ("Could not open file!");  # file containing selected training set
		for (my $i = 0; $i<=$#clusters; $i++) {
			if (defined $clusters[$i]) {
				...
				if($size[$i]>0 ){
                                            print "large centroid $temp ---$size[$i] label $labels[$mininst] --- ($size[$i] - $clustrel[$i]) $temp\n";
																						$temp++;
                                }
				...
				if($labels[$mininst] ==0.0){
                                    $totalN=$totalN+1;
                                }else{
                                    $totalP=$totalP+1;
                                }
                                #print "$totalN $totalP ----\n";
                                if($labels[$mininst] eq 5.0 ){
                                    print "pulando..\n";
                                }else{
		...
		# sort final training file by queryid (for L2R datasets)
		if ($hasqid eq "") {
			system("cat $resultfolder/selectedset.tmp | sort -t \: -n -k 2 > $resultfolder/selectedset.txt");
			system("rm -f $resultfolder/selectedset.tmp");
		}
		else {
			system("mv $resultfolder/selectedset.tmp $resultfolder/selectedset.txt");
		}

	}

	# finish execution and write epsilon for each batch
	if ($currsize < $selsize) {
