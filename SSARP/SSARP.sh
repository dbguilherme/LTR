...
#while [ $i -le 1 ]; do
	...
	if [ ! -d $FOLDER ]; then # soh executa se o diretorio nao existe ainda
		...
		if [ ! -f  ../train.txt.arff ]; then 
		    ...
		    ../../convert_letor_to_weka /tmp/firstRoundSelection.txt $numfeatures > ../train.txt.arff
		    ../../convert_letor_to_weka ../test.txt $numfeatures >  ../test.txt.arff
...
rm /tmp/SecondRoundSelection.txt
cat Fold1/$FOLDER/composite_train_uniq$suffix.txt | awk '{ print $1 }'  | while read instance; do  sed  -n  "$instance"p  /tmp/firstRoundSelection.txt; done >> /tmp/SecondRoundSelection.txt;
